...
const APP_BUILD_STAMP = "ui-revamp-darkmode-v2";
const SEARCH_RESULT_LIMIT_OPTIONS = new Set([5, 10, 25, 100]);
...
const THEME_STORAGE_KEY = "weeknightThemeV1";
...
function setBuildStamp() {
  ...
  buildStampEl.textContent = `Build: ${APP_BUILD_STAMP}`;
}
...
function applyTheme(theme) {
  ...
  themeToggleEl.textContent = isDark ? "☀️ Light mode" : "🌙 Dark mode";
...
async function loadRecipes(query) {
  ...
  try {
    ...
    displayMeals.slice(0, resultLimit).forEach((meal) => {
    ...
    const shownCount = Math.min(resultLimit, displayMeals.length);
    setStatus(
...
function applyStrictPreFilters(meals, rawQuery) {
  ...
  const userRequestedDinnerFormat = RECIPE_FILTER_CONFIG.preferredDinnerFormats.some((term) =>
    userRequestedTerms.has(term)
  );
  ...
  const strictMatches = meals.filter((meal) => {
    ...
    if (!hasAllowedProtein && !userRequestedDinnerFormat) {
      return false;
    }
...
function buildSearchTerms(query) {
  ...
  words.forEach((word) => {
    ...
    (TERM_ALIASES[word] || []).forEach((alias) => terms.add(alias));

    const canonical = getCanonicalSearchTerm(word);
    if (canonical && canonical !== word) {
      terms.add(canonical);
      (TERM_ALIASES[canonical] || []).forEach((alias) => terms.add(alias));
    }
  });
...
function buildQuerySignals(rawQuery) {
  ...
  const requiredTerms = cleaned
    ...
    .map((word) => getCanonicalSearchTerm(word));

  const specialMatchers = [];
  if (/\b(ground beef|minced beef|beef mince)\b/.test(cleaned) || (cleaned.includes("ground") && cleaned.includes("beef"))) {
    specialMatchers.push((text) => /\bbeef\b/.test(text) && /\b(mince|minced|ground beef|beef mince|minced beef)\b/.test(text));
  }
...
function getCanonicalSearchTerm(term) {
  if (!term) return term;
...
function getSearchResultLimit() {
  const selectedValue = Number(resultLimitEl?.value || DEFAULT_SEARCH_RESULT_LIMIT);
  return SEARCH_RESULT_LIMIT_OPTIONS.has(selectedValue)
    ? selectedValue
